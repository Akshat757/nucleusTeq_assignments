...
// Single DOMContentLoaded listener for initialization.
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Load owner-specific data if corresponding elements are present.
...
function showRestaurantForm() {
    ...
    // Clear hidden restaurant ID (for new restaurant creation)
...
// Redirect to Manage Menus page with query parameter for the selected restaurant.
...
document.getElementById("restaurantForm")?.addEventListener("submit", async (e) => {
    ...
    try {
        ...
        closeRestaurantForm();
        await loadOwnerRestaurants();
        // If Manage Menus page is open, update its restaurant dropdown.
        if (document.getElementById("restaurantSelect")) {
            await loadOwnerRestaurantsForSelect();
        }
    } catch (error) {
...
async function editRestaurant(restaurantId) {
    try {
        ...
        alert("Failed to load restaurant details: " + error.message);
    }
}
...
async function loadOwnerRestaurantsForSelect() {
    ...
    try {
        ...
        if (select) {
            ...
            // If a query parameter exists in URL, preselect that restaurant.
        ...
        alert("Failed to load restaurants for menu management: " + error.message);
    }
}
        ...
        fetch(`${API_BASE}/menuitems/${menuItemId}`, {
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    location.reload(); // Reload to update the menu
                } else {
